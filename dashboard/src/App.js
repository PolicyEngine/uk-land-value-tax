...
function BaselineSection() {
  return (
    <section className="dashboard-section">
      <SectionTitle prose={<p>This analysis estimates UK land values, examines how they are distributed across households, and simulates replacing council tax with a land value tax at various rates. PolicyEngine <a href="https://github.com/PolicyEngine/uk-land-value-tax" target="_blank" rel="noopener noreferrer">estimates</a> UK land values and simulates tax reforms. See the <a href="#methodology">methodology note</a> below for details on how land values are derived. The ONS <a href="https://www.ons.gov.uk/economy/nationalaccounts/uksectoraccounts/datasets/thenationalbalancesheetestimates" target="_blank" rel="noopener noreferrer">reports</a> total UK land at £{Number(data.ons_comparison.ons_2024_total_tn).toFixed(1)}tn for 2024, of which £{Number(data.ons_comparison.ons_2024_household_tn).toFixed(1)}tn is household land and £{(data.ons_comparison.ons_2024_corporate_tn + data.ons_comparison.ons_2024_government_tn).toFixed(1)}tn is non-household (corporate and government). The model estimates total UK land at £{Number(data.baseline.total_land_tn).toFixed(1)}tn, and total UK household wealth at £{Number(data.baseline.total_wealth_tn).toFixed(1)}tn, of which land accounts for {Number(data.baseline.land_pct_of_wealth).toFixed(1)}%.</p>}>
...
function ReformSection() {
  ...
  return (
    <section className="dashboard-section">
      ...
      <p className="subsection-prose">Council tax is a banded property tax that PolicyEngine UK estimates raises £{ctBn} billion annually across the UK. The OBR estimates <a href="https://obr.uk/forecasts-in-depth/tax-by-tax-spend-by-spend/council-tax/" target="_blank" rel="noopener noreferrer">£50.9 billion for England alone</a> in 2025-26. A land value tax levies a flat percentage on land values. At the budget-neutral rate of {data.council_tax_replacement.required_lvt_rate_pct}%, LVT raises the same total revenue for government as council tax, so there is no change to public spending. Figure 2 shows net revenue at each rate.</p>
