...
function BaselineSection() {
  return (
    <section className="dashboard-section">
      <SectionTitle prose={<p>PolicyEngine UK <a href="https://github.com/PolicyEngine/uk-land-value-tax" target="_blank" rel="noopener noreferrer">estimates</a> land values across UK households, examines their distribution, and simulates replacing council tax with a land value tax (LVT) at various rates. See the <a href="#methodology">methodology note</a> below for details on how land values are derived. The ONS <a href="https://www.ons.gov.uk/economy/nationalaccounts/uksectoraccounts/datasets/thenationalbalancesheetestimates" target="_blank" rel="noopener noreferrer">reports</a> total UK land at £{Number(data.ons_comparison.ons_2024_total_tn).toFixed(1)}tn for 2024, of which £{Number(data.ons_comparison.ons_2024_household_tn).toFixed(1)}tn is household land and £{(data.ons_comparison.ons_2024_corporate_tn + data.ons_comparison.ons_2024_government_tn).toFixed(1)}tn is non-household (corporate and government). The model estimates total UK land at £{Number(data.baseline.total_land_tn).toFixed(1)}tn and total UK household wealth at £{Number(data.baseline.total_wealth_tn).toFixed(1)}tn, of which land accounts for {Number(data.baseline.land_pct_of_wealth).toFixed(1)}%.</p>}>
